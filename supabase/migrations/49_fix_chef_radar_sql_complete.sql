-- Complete fix for Chef Radar matching function
-- This version properly handles empty inventory and uses LEFT JOIN instead of CROSS JOIN
...
begin
    ...
    -- If no inventory, return empty (AI generation will handle it)
    ...
    recipe_matches as (
        select
            ...
            (
                select array_agg(distinct ui.original_name)
                from user_inventory ui
                where exists (
                    select 1
                    from jsonb_array_elements(r.ingredients) as ing
