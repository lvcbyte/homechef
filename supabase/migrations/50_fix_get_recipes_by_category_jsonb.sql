-- Fix get_recipes_by_category to properly handle JSONB dietary_restrictions
-- This migration ensures the function accepts both JSONB and text[] for dietary_restrictions

-- Drop and recreate the function with proper JSONB handling
drop function if exists public.get_recipes_by_category(text, integer, uuid, text, text, text[]);
