-- Fix match_product_catalog to work better and limit to 10 results
-- This ensures products like "bananen" are found correctly
...
begin
    ...
    with matches as (
        select
            ...
            case
                ...
                -- Word match in product name (score 50) - improved for single words
                when exists (
                    select 1 
                    from unnest(search_words) as word
                    where length(word) >= 2  -- Match words with 2+ chars (was > 2)
                ...
                -- High similarity (score 40) - lowered threshold
                when similarity(lower(pc.product_name), search_lower) > 0.4 then 40.0
                -- Medium similarity (score 30) - lowered threshold
                ...
                -- Low similarity (score 10) - lowered threshold for better results
        ...
        where pc.is_available = true
          and (
            ...
            -- Word match - improved to match words with 2+ chars
            or exists (
                select 1 
                from unnest(search_words) as word
                where length(word) >= 2  -- Changed from > 2 to >= 2
                and lower(pc.product_name) like '%' || word || '%'
            )
            -- Trigram match (fuzzy)
            or pc.product_name % search_term
            -- Similarity match - lowered threshold from 0.2 to 0.15
    ...
    limit 10; -- Maximum 10 results as requested
end;
$$;
