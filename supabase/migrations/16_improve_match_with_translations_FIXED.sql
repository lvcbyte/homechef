...
-- NOTE: Run migration 15_product_translations_FIXED.sql FIRST!
...
begin
    ...
    -- Get search variants (original + translations) if function exists
    if translation_func_exists then
        ...
        exception when others then
            -- If translation functions fail, just use original term
            search_variants := ARRAY[search_lower];
        end;
    else
        search_variants := ARRAY[search_lower];
    ...
    with matches as (
        select
            ...
            case
                ...
                -- Translation match (score 45) - only if variants exist
        ...
        where pc.is_available = true
          and (
            ...
            -- Match on translated variants (if they exist)
            ...
            or (translation_func_exists and exists (
                select 1 from pg_proc p
                join pg_namespace n on p.pronamespace = n.oid
                where n.nspname = 'public' and p.proname = 'translate_to_dutch'
