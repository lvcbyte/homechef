-- Improve archetype filtering to show more distinct recipe sets
-- Make archetype preferences more pronounced and ensure recipes are always shown
...
declare
    ...
    -- Archetype preferences mapping - more distinct
...
begin
    ...
    -- Map archetype to preferences - more distinct tags
    ...
    select
        ...
        rm.difficulty,
        rm.servings,
        -- Enhanced match score with archetype bonus (more pronounced)
        (
            rm.base_match_score * 100.0
            + case
                -- Higher bonus for archetype-preferred tags (more distinct)
                when exists (
                    select 1 from unnest(v_archetype_tags) as tag
                    where tag = any(rm.tags)
                ) then 25.0  -- Increased from 15.0
                else 0.0
            end
    ...
    where rm.base_match_score > v_match_threshold
       ...
       -- Always show recipes that match archetype tags, even with low ingredient match
       or (array_length(v_archetype_tags, 1) > 0 and exists (
           select 1 from unnest(v_archetype_tags) as tag
           where tag = any(rm.tags)
       ))
...
-- Update get_trending_recipes to prioritize archetype-matched recipes
