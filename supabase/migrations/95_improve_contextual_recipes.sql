-- Improve Contextual Recipes Function
-- Better matching for cold weather and comfort food

-- Update function to better match comfort food categories
...
BEGIN
  ...
  -- Weather-based filtering (improved)
  IF p_weather_condition = 'rain' AND p_time_of_day = 'dinner' THEN
    -- Rain + evening = Comfort Food
    v_categories := v_categories || ARRAY['Comfort Food', 'Stoofpot', 'Soep', 'Pasta', 'Rijst'];
    v_tags := v_tags || ARRAY['comfort', 'warm', 'stevige maaltijd', 'stoofpot', 'comfort food', 'pasta', 'rijst'];
  ELSIF p_weather_condition = 'rain' THEN
    -- Rain (any time) = Comfort Food
    v_categories := v_categories || ARRAY['Comfort Food', 'Soep'];
    v_tags := v_tags || ARRAY['comfort', 'warm', 'comfort food', 'soep'];
  ELSIF p_weather_condition = 'warm' THEN
    -- Warm weather = Salads and light meals
    v_categories := v_categories || ARRAY['Salade', 'Lichte Maaltijd', 'BBQ'];
    v_tags := v_tags || ARRAY['licht', 'fris', 'verfrissend', 'salade', 'light', 'bbq', 'grill'];
  ...
  WHERE
    ...
    (
      ...
      OR
      -- Fallback: if no specific matches, return general recipes
...
-- Add comment
COMMENT ON FUNCTION get_contextual_recipes IS 'Returns recipes filtered by time of day and weather conditions for contextual recommendations. Improved matching for cold weather and comfort food.';
