/**
 * Direct web scraper for Delhaize Belgium
 * Scrapes delhaize.be directly and imports to Supabase
 */
...
async function scrapeDelhaizeCategory(categoryUrl: string) {
  try {
    ...
    // Delhaize product structure - adjust selectors based on actual website
    $('.product-item, .product-card, .product-tile, [data-product], .product, .product-wrapper').each((_, element) => {
      const $el = $(element);
      ...
      const product = {
        id: $el.attr('data-product-id') || 
              ...
              `delhaize-${crypto.randomUUID()}`,
        name: $el.find('.product-name, .product-title, .name, h3, h4, .title, .product-link').first().text().trim() || 
              $el.attr('data-product-name') || 
              $el.find('[data-product-name]').attr('data-product-name') || 
              'Unknown',
        price: parseFloat(
          ($el.find('.price, .product-price, .current-price, [data-price], .price-value').first().text().trim() || 
           $el.attr('data-price') || 
           '0')
          .replace(/[^\d,.-]/g, '')
          .replace(',', '.')
        ...
        barcode: $el.attr('data-barcode') || 
                 ...
                 null,
        unitSize: $el.find('.unit-size, .size, .unit, .product-size').first().text().trim() || null,
        brand: $el.find('.brand, .product-brand').first().text().trim() || 'Delhaize',
      };
...
async function scrapeDelhaizeCatalog() {
  console.log('🕷️  Scraping Delhaize.be directly...');
  
  const baseUrl = 'https://www.delhaize.be';
  
  // Delhaize category pages - adjust URLs based on actual website structure
  const categoryUrls = [
    ...
    `${baseUrl}/nl/shop/huishoud`,
    `${baseUrl}/nl/shop/verzorging`,
  ];
