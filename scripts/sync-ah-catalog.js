...
if (!SUPABASE_URL || !SERVICE_ROLE_KEY || !APIFY_TOKEN) {
  ...
  console.error('   SUPABASE_URL:', SUPABASE_URL ? `✅ (${SUPABASE_URL.substring(0, 30)}...)` : '❌');
  console.error('   SERVICE_ROLE_KEY:', SERVICE_ROLE_KEY ? `✅ (${SERVICE_ROLE_KEY.substring(0, 20)}...)` : '❌');
  console.error('   APIFY_TOKEN:', APIFY_TOKEN ? `✅ (${APIFY_TOKEN.substring(0, 20)}...)` : '❌');
  console.error('\n💡 Tip: Make sure to export the variables in your shell:');
  console.error('   export SUPABASE_URL="..."');
  console.error('   export SERVICE_ROLE_KEY="..."');
  console.error('   export APIFY_TOKEN="..."');
  process.exit(1);
}
